...
import {
  ...
  stageTuiRuntime,
} from "./mgcode-distribution/assets.mjs";
import { installBundledNodeRuntime } from "./mgcode-distribution/bundled-node.mjs";
import { installScriptSource } from "./mgcode-distribution/installer.mjs";

const root = resolve(import.meta.dirname, "..");
const defaultOutDir = resolve(root, "dist", "mgcode");
...
async function stageZCodePackage({ packageRoot, version }) {
  ...
  const agentProvider = resolve(root, "apps/mgcode-cli/zcode-cli/packages/cli/dist/provider");
  ...
  await cp(
    resolve(root, "apps/mgcode-cli/zcode-cli/packages/cli/dist/THIRD-PARTY-NOTICES.md"),
  ...
  await cp(resolve(root, "scripts/mgcode-distribution/runner.mjs"), runner);
