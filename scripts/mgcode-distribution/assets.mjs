...
import {
  collectSeaTuiAssets,
  seaTuiAssetPrefix,
} from "../../apps/mgcode-cli/zcode-cli/packages/cli/scripts/sea-tui-assets.mjs";
import { supportedTargets } from "../../apps/mgcode-cli/zcode-cli/packages/cli/scripts/sea-targets.mjs";
  ...
  try {
    for (const target of supportedTargets) {
      const { assets, manifest } = await collectSeaTuiAssets({
        root: resolve(root, "apps/mgcode-cli/zcode-cli"),
        stagingDirectory,
        target,
      });
...
export async function copyRuntimeNodeModules(packageRoot) {
  ...
  await copyRuntimePackageTree({
    ...
    requireFrom: createRequire(resolve(root, "apps/mgcode-cli/zcode-cli/packages/cli/package.json")),
    seen: new Set(),
  });
}
