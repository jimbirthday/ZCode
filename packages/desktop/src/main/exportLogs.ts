...
function getZCodeCliDir() {
  return join(homedir(), ".mgcode", "cli");
}
...
function getCuaHelperRunDir() {
  return join(homedir(), ".mgcode", "computer-use", "run");
}
...
function isExcludedRelativePath(relativePath: string): boolean {
  ...
  // 过去显式收集 ~/.mgcode/cli/debug 会把这类上下文带进手动导出和反馈完整日志，这里按目录段统一跳过。
  ...
  // ~/.mgcode/v2/dev 保存 stdio-traffic 等高频协议流，真实机器上会累计到 GB 级。
...
function shouldApplyLogExportRetention(archivePath: string): boolean {
  ...
  if (
    ...
    normalizedArchivePath.startsWith(".mgcode/cli/log/")
  ) {
    return true;
  }
...
async function createLogArchiveArtifacts(
  ...
  // 二者都不在 ~/.mgcode/cli/log 下，需要额外收集才能完整还原现场。
  await collectLogArchiveFile(
    join(zcodeCliDir, "config.json"),
    posix.join(".mgcode", "cli", "config.json"),
    files,
  );
  await collectLogArchiveFilesFromDirectory(
    join(zcodeCliDir, "rollout"),
    posix.join(".mgcode", "cli", "rollout"),
    visitedDirs,
    files,
  );
  ...
  // ~/.mgcode/computer-use/run/<socket>.exit.log，既不在 app data 也不在 ~/.mgcode/cli 下。
  // 同目录下有 .tokens broker 凭据，因此按文件名白名单只收 *.exit.log，不递归该目录。
  await collectLogArchiveFilesByName(
    getCuaHelperRunDir(),
    posix.join(".mgcode", "computer-use", "run"),
...
export async function createFeedbackLogArchiveFromExportLogs(
  ...
  return createFeedbackDiagnosticArchive({
    sources: [
      ...
      { directory: getZCodeCliLogDir(), archivePrefix: ".mgcode/cli/log" },
      {
        directory: getCuaHelperRunDir(),
        archivePrefix: ".mgcode/computer-use/run",
        exitLogsOnly: true,
      },
    ],
