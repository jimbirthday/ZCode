...
async function clearAllDataAndRelaunch(options: {
  ...
  const { response } = await dialog.showMessageBox({
    ...
    message: "确定要清除所有数据吗？",
    detail:
      "将删除 ~/.mgcode/v2（配置、凭据、日志）和浏览器缓存（localStorage）。操作不可恢复，清除后应用将自动重启。",
  });
  if (response !== 1) {
    return;
  }
  ...
  try {
    ...
    options.logger.info("[clear-all-data] deleted ~/.mgcode/v2");
  } catch (error) {
    options.logger.error("[clear-all-data] failed to delete ~/.mgcode/v2:", error);
  }
