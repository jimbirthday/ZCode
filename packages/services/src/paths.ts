...
export function getZCodeDataRootDir(): string {
  return join(getDataBaseDir(), PRODUCT_DATA_DIR_NAME);
}

/** 非项目对话共享的真实工作目录；默认 ~/.mgcode/workspace/default。 */
export function getConversationWorkspaceDir(): string {
  return join(getZCodeDataRootDir(), "workspace", "default");
}

/** {dataBaseDir}/.mgcode/v2 */
...
/** ~/.mgcode/v2/sessions/{workspaceHash} */
...
/** ~/.mgcode/v2/sessions/{workspaceHash}/{taskId}.json */
...
/** ~/.mgcode/v2/sessions/{workspaceHash}/{taskId}.deleted.json */
...
/**
 * Copy the .mgcode/v2 data directory from one base dir to another.
...
export async function copyDataDirectory(oldBaseDir: string, newBaseDir: string): Promise<void> {
  const oldDir = join(oldBaseDir, PRODUCT_DATA_DIR_NAME, "v2");
  const newDir = join(newBaseDir, PRODUCT_DATA_DIR_NAME, "v2");
