...
} from "@zcode/server/remote/mgcodeAgentBundleWrapper.js";
import {
  deployRemoteAgentWrapper,
  isWslBackend,
} from "@zcode/server/remote/mgcodeAgentWrapperDeploy.js";
import {
...
} from "@zcode/server/remote/mgcodeAgentOfficialPluginAssets.js";
import { repairLegacyRemoteOfficialPluginDirectoryPermissions } from "@zcode/server/remote/mgcodeAgentOfficialPluginPermissionRepair.js";

const DEV_AGENT_BUNDLE_RELATIVE_PATH = "apps/mgcode-cli/zcode-cli/packages/cli/dist/zcode.cjs";
...
export async function deployDevelopmentZCodeAgentRuntime(
  ...
  // 本地修改 apps/mgcode-cli/zcode-cli 后，远端测试仍运行滞后的发布包。这里改为上传 dev 启动时刚构建的
