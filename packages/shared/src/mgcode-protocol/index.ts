...
// session/workspace state snapshot 投影等（承重类型已迁 mgcode-protocol-legacy-types.ts）。
...
// CLI bootstrap 旧协议 server（mgcode-protocol/server-operations、plugins、session-mapper 等）、
...
} from "../mgcode-protocol-legacy-types.js";
...
const zcodeComputerUseToolScheduledEventSchema = zcodeComputerUseOperationEventBaseSchema.extend({
  ...
  // bootstrap/src/mgcode-protocol/computer-use-operation-event.ts 的 usesComputerUse）。
...
// `<cwd>/.mgcode/workflows/`（挂载时快照会漏掉手改的文件）。形状与 @zcode/contracts 的
...
// 作用域两档：项目档落 `<cwd>/.mgcode/workflows/`、全局档落 agent 机器的 `~/.mgcode/workflows/`。作用域由文件所在目录推得，frontmatter 不存 scope。
...
export const zcodeWorkflowsListParamsSchema = z
  .object({
    workspace: zcodeWorkspaceRefSchema,
    // 缺省即 `project`（本项目档）。给 `global` 时改扫本机 `~/.mgcode/workflows/`；此时 `workspace`
