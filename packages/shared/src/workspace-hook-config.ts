...
export type WorkspaceHookConfigFileKind = "zcode.json" | ".mgcode/config.json" | "explicit";
...
function buildWorkspaceHookCandidatePaths(directories: readonly string[]): string[] {
  return directories.flatMap((directory) => [
    ...
    join(directory, ".mgcode", "config.json"),
  ]);
}
...
export function createWorkspaceHookSourceInput(input: {
  ...
  return {
    canonicalPath,
    baseDir: basename(configDirectory) === ".mgcode" ? dirname(configDirectory) : configDirectory,
    ...
    editable:
      !explicitProjectConfig &&
      canonicalPath === resolve(input.workingDirectory, ".mgcode", "config.json"),
    hooks: input.hooks,
  };
}
