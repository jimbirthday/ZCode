 ...
 *
 * 项目组传 `workspacePath`（拼出 `<ws>/.mgcode/workflows`）；全局组传 `directory`（协议 list 回的
 * 绝对目录，即 `~/.mgcode/workflows`），二者择一——`directory` 优先。
 */
export function useSavedWorkflowsDirectoryWatch({
  ...
  useEffect(() => {
    ...
    void fileWatcherService
      ...
      .catch((error: unknown) => {
        logger.debug("[SavedWorkflows] 监听 .mgcode/workflows 失败（目录可能尚不存在）", {
          path: directoryPath,
