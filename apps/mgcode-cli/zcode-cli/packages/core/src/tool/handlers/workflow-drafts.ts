...
// 这里写一个。
//
// 落点 `<cwd>/.mgcode/workflow-drafts/`，与 `.mgcode/workflows/`（用户保存的定义）、
// `.mgcode/workflow-runs/`（每个 run 的编译入口）平级。目录自带一份 `.gitignore: *`，写法与
...
interface WriteWorkflowDraftInput {
  /**
   * 会话工作目录；草稿落在它的 `.mgcode/workflow-drafts/` 下。缺席即宿主没有工作目录概念
