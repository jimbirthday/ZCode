...
function buildInitAgentsPrompt(params: {
  ...
  return [
    ...
    `- Existing hidden instruction candidates: ${join(params.workingDirectory, ".mgcode", "AGENTS.md")} and ${join(params.workingDirectory, ".agents", "AGENTS.md")}`,
    "- File name must be exactly AGENTS.md.",
    "- This command targets the current workspace only. Do not write ~/.mgcode/AGENTS.md.",
    additionalInstructions,
    "",
    "Process:",
    "1. First check whether .mgcode/AGENTS.md or .agents/AGENTS.md exists in the workspace. If either exists, tell the user they already have an instructions file, mention the path found, and stop without creating a new AGENTS.md.",
