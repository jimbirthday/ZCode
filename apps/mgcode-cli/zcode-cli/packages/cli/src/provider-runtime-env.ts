...
export async function prepareCliProviderRuntimeEnv(
  ...
  const personalFilePath =
    explicitPersonal ?? join(dataBaseDir, ".mgcode", "v2", PERSONAL_PROVIDER_CONFIG_FILE_NAME);
  ...
  const cachePaths = resolveZCodeBuiltinCachePaths({
    environmentConfigRoot: join(dataBaseDir, ".mgcode", "v2"),
    platform,
    appVersion,
    zcodeEndpointOrigin,
  });
...
async function resolveBundledZCodeBuiltinProviderConfig(input: {
  ...
  if (input.sea?.isSea()) {
    ...
    return materializeZCodeBuiltinProviderConfig({
      environmentConfigRoot: join(input.dataBaseDir, ".mgcode", "v2"),
      content,
    });
  }
