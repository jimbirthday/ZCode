...
const CreateWorkflowModelInputSchema = z
  .object({
    name: z
      ...
      .optional()
      .describe(
        "Short display label for this run, in the user's language (\"PR review\", \"代码评审\"). Always pass it for an inline script: it labels the run everywhere and names its draft file under .mgcode/workflow-drafts/. Defaults to the saved workflow's name when running a saved workflow.",
      ),
    script: z
      .string()
      .optional()
