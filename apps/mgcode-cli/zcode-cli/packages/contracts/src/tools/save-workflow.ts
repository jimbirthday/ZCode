...
export const SaveWorkflowInputSchema = z
  .object({
    ...
    scope: SavedWorkflowScopeSchema.describe(
      'Where the workflow is saved. "project" when the script references this repository\'s files, commands, conventions or directory layout; "global" when it depends on nothing in this project and should be available from every project (saved under ~/.mgcode/workflows). Decide every time; there is no default.',
    ),
