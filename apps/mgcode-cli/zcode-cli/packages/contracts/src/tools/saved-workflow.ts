...
/** 项目作用域的存放目录（相对会话工作目录）。 */
export const SAVED_WORKFLOW_PROJECT_DIR = `${PRODUCT_DATA_DIR_NAME}/workflows`;

/**
 * 草稿目录（相对会话工作目录）。模型在两次提交之间就地编辑的脚本文件落在这里，是
 * `.mgcode/workflows/`（用户保存的定义）的兄弟目录，机器自有、自带 `.gitignore: *`。
 */
export const WORKFLOW_DRAFTS_DIR = `${PRODUCT_DATA_DIR_NAME}/workflow-drafts`;

/**
 * 全局作用域的存放目录（相对 agent 进程的家目录）。落点 `~/.mgcode/workflows/<name>.dwf.ts`
 * ——与 legacy Workflow 工具的用户根同一处，对所有项目可见。
 */
export const SAVED_WORKFLOW_GLOBAL_DIR = `${PRODUCT_DATA_DIR_NAME}/workflows`;
...
/**
 * 作用域。两档：`project` 落在项目的 `.mgcode/workflows/`，只在那个项目里可见；`global`
 * 落在 `~/.mgcode/workflows/`（agent 进程的家目录），对所有项目可见。一个文件的作用域由它所在的目录推得，frontmatter 不存。
 */
