 ...
 * `<cwd>/.mgcode/workflow-runs/<runId>.mjs`，spawn 时命令行只剩这条路径。
 *
 * 裁决：
 *   - 位置 `.mgcode/workflow-runs/`，与项目级 saved workflow 的 `.mgcode/workflows/` 同级不混放；
...
export interface HarnessWarning {
  ...
  /** 本想写入的项目目录（`<cwd>/.mgcode/workflow-runs`）。 */
  projectDir: string;
  /** 实际回落到的目录。 */
  fallbackDir: string;
  /** 项目目录写入失败的错误文本。 */
...
export function workflowRunsDir(cwd: string): string {
  return join(cwd, ".mgcode", "workflow-runs");
}
